...
def wakeLoop () :
	while True :
		...
		elif speech.ratio(command, 'stop') > 60 or speech.ratio(command, 'that\'s all') > 60 or speech.ratio(command, 'that is it') > 60 or speech.ratio(command, 'enter hybernation mode') > 60 or speech.ratio(command, 'start hybernation') > 60 or speech.ratio(command, 'sleep') > 60 :
			speech.say(f'Entering hybernation mode, say "open {const.name}" again, to start interaction')
			break